...
class FafuRobotController:
    ...
    def _enter_operation(self, action: str, busy_state: RobotState) -> bool:
        ...
        if self._op_depth > 0:
            ...
            self._op_depth += 1
            return False
        # Gripper is an independent actuator. Force-grasp must be legal
        # while joints are in a servo session; flipping SERVOING→GRASPING
        # would make the next servo_start think motors are disabled and
        # motor_reset (PWM gap: arm drops, jaws spring open).
        if (
            busy_state is RobotState.GRASPING
            and self._state is RobotState.SERVOING
        ):
            self._require_gripper_ready(action)
            self._op_depth += 1
            return False
    ...
    def _joints_accept_servo(self) -> bool:
        """True if every *joint* is already in a mode that can take servo frames.

        ``is_enabled`` requires MODE_POSITION (``0x0A``) on **all** motors,
        including the gripper. After ``servo_end`` the joints are typically
        left in MIT leftover ``0x0B``; ``enable()`` then ``motor_reset``s
        every motor, which cuts PWM for tens of milliseconds (arm drops,
        gripper springs open). ``0x0A`` and ``0x0B`` both accept the
        ``0x8090`` / ``0x8093`` frames ``servo_j`` sends, so skip the reset.
        """
        live = {self.MODE_POSITION, self.MODE_MIT}
        for mid in self._joint_motor_ids:
            s = self._ht.get_cached_state(mid)
            if s is None:
                s = self._ht.read_motor_state(mid, 0.08)
            if s is None or int(s.mode) not in live:
                return False
        return True
    ...
    def servo_start(self, opts: Optional[ServoOpts] = None) -> None:
        ...
        if not self.is_enabled:
            if self._joints_accept_servo():
                print(
                    "[FafuRobot] servo_start: leftover 0x0A/0x0B control mode; "
                    "skip enable()/motor_reset (reset cuts PWM and the arm drops)"
                )
            else:
