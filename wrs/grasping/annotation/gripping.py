...
def define_gripper_grasps(gripper,
                          ...
                          toggle_dbg=False):
    """
    返回的 ``Grasp.ac_pos``/``ac_rotmat`` 为 **世界系** 下夹爪 acting center 位姿；若用于
    ``PickPlacePlanner`` 或物体位姿与规划时不一致，须先对 ``obj_cmodel`` 做
    ``wrs.grasping.grasp.grasp_collection_world_to_object_frame``，或使用
    ``gpa.plan_gripper_grasps(..., in_object_frame=True)``。
