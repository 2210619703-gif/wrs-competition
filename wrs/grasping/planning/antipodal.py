...
import wrs.grasping.grasp as ggg
...
def plan_gripper_grasps(gripper,
    ...
    :param in_object_frame: 若为 True，将各 grasp 的 ``ac_pos``/``ac_rotmat`` 从世界系转物体系
        （与 ``PickPlacePlanner``/pickle 中常用约定一致）。原为 False 以保持本模块 ``__main__`` 等
        中 ``grip_at_by_pose(grasp.ac_pos, …)`` 的调试用法；物体在原点且``rotmat=I`` 时与 False 无差别。
    ...
    if in_object_frame:
        grasp_collection = ggg.grasp_collection_world_to_object_frame(
            grasp_collection, obj_cmodel.pos, obj_cmodel.rotmat)
