...
if __name__ == "__main__":
    ...
    rs_pipe = RealSenseD405(resoultion='mid')
    # the first frame contains no data informationF
    rs_pipe.get_pcd_texture_depth()
    rs_pipe.get_pcd_texture_depth()
    rbtx = RealmanArmController(ip='192.168.3.18', has_gripper=False)
    rbt = Realman(rotmat=rm.rotmat_from_euler(0, np.radians(90), 0))


    # 加载初始标定矩阵
    init_mat = load_calibration_matrix_from_json("manual_calibration_realman.json")
    mgm.gen_frame(ax_length=1).attach_to(base)
    xarm_mc = RealmanManualCalib(rbt_s=rbt ,rbt_x=rbtx, sensor_hdl=rs_pipe,
