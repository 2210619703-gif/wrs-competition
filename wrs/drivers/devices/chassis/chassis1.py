'''
Author: wang yining
Date: 2025-08-01 13:48:17
LastEditTime: 2025-08-01 13:41:01
FilePath: /remote_fs/wrs/drivers/devices/chassis/chassis1.py
Description: 
...
class ChassisControl:
    ...
    def validate(self):
        ...
        if not 0.0 <= self.target_speed <= 1000.0:
...
def control_chassis(run_time: float, water_pump: bool, remote_enable: bool, 
                   ...
                   chassis_mode: int, target_angle: int) -> None:
    """
    控制底盘运动
    ...
    if not isinstance(target_speed, (int, float)) or not 0 <= target_speed <= 1000:
...
if __name__ == "__main__":
    ...
    #停止
    control_chassis(run_time=3,water_pump=False,remote_enable=True,gear_position=1,target_speed=10,chassis_mode=1,target_angle=0)
    control_chassis(run_time=1,water_pump=False,remote_enable=False,gear_position=2,target_speed=0,chassis_mode=7,target_angle=0)
