...
def _get_panda_base():
    """Panda3D 可视化用的 ShowBase 实例，脚本常在 __main__ 里设 ``base``；无则返回 None (headless)。"""
    main = sys.modules.get("__main__")
    if main is not None and getattr(main, "base", None) is not None:
        return main.base
    return None
...
class RRTConnect(rrt.RRT):
    ...
    def plan(self,
            ...
            if toggle_dbg:
                ...
                if b is not None:
                    ...
                    for point in start_cdresult[1]:
                        mgm.gen_sphere(pos=point, radius=.005).attach_to(b)
                    self.robot.gen_meshmodel(toggle_cdprim=True, alpha=.3).attach_to(b)
                    # 切勿在此 base.run()：会吞掉主循环，上层仿真表现为卡死/退出
                    print("RRT: (toggle_dbg) 已画出 start 碰撞接触点，请手动检查窗口。")
                else:
                    print("RRT: (toggle_dbg) 无 Panda ``base``，跳过球体/模型可视化。")
            return None
            ...
            if toggle_dbg:
                ...
                if b is not None:
                    ...
                    for point in goal_cdresult[1]:
                        mgm.gen_sphere(pos=point, radius=.005).attach_to(b)
                    self.robot.gen_meshmodel(toggle_cdprim=True, alpha=.3).attach_to(b)
                    print("RRT: (toggle_dbg) 已画出 goal 碰撞接触点，请手动检查窗口。")
                else:
                    print("RRT: (toggle_dbg) 无 Panda ``base``，跳过球体/模型可视化。")
            return None
        ...
        mot_data = rrt.motd.MotionData(self.robot)
        b = _get_panda_base()
        if b is not None and getattr(b, "toggle_mesh", True):
