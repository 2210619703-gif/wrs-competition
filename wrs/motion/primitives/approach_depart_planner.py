...
_DEFAULT_RRT_EXT_DIST = 0.4
_DEFAULT_RRT_SMOOTHING_N_ITER = 100
# Per-RRT time budget (seconds). The original value 100 let one infeasible grasp
# stall the whole plan for a very long time (looked like a freeze). A small budget
# makes a hard grasp fail fast so planning moves on to the next grasp immediately.
_DEFAULT_RRT_MAX_TIME = 6.0
...
class ADPlanner(object):
    ...
    def gen_approach(self,
        ...
        if use_rrt:
            ...
            start2app = self.rrtc_planner.plan(
                start_conf=start_jnt_values,
                goal_conf=linear_app.jv_list[0],
                obstacle_list=obstacle_list + object_list,
                ext_dist=_DEFAULT_RRT_EXT_DIST,
                max_time=_DEFAULT_RRT_MAX_TIME,
                smoothing_n_iter=_DEFAULT_RRT_SMOOTHING_N_ITER,
            )
            if ee_values is not None:
    ...
    def gen_depart(self,
        ...
        if use_rrt:
            ...
            if ee_values is not None:
                ee_values_bk = self.robot.get_ee_values()
                self.robot.change_ee_values(ee_values=ee_values)
            dep2end = self.rrtc_planner.plan(
                start_conf=linear_dep.jv_list[-1],
