#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2025/10/21 11:59
# @Author : ZhangXi
#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2025/10/14 19:57
# @Author : ZhangXi
#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2025/6/7 11:11
...
class PigInjector(gpi.GripperInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), ik_solver='a', name='injector',
        ...
        real_offset = np.array([0.017, 0.03, 0.005])
        ...
        self.loc_acting_center_pos = np.array([.18015519, -.0017, .30380530]) + real_offset
    ...
    def _back_mdl_cdnp(self, ex_radius):
        collision_node = CollisionNode('injector_pig')
        # 枪体
        collision_primitive_c0 = CollisionBox(Point3(.18, 0, 0.18),
                                              x=0.045 + ex_radius, y=.05 + ex_radius, z=.1 + ex_radius)
        collision_node.addSolid(collision_primitive_c0)
        # L型竖杆
        collision_primitive_c1 = CollisionBox(Point3(.068, .0, .1),
                                              x=0.075 + ex_radius, y=0.015 + ex_radius, z=0.015 + ex_radius)
        ...
        # 注射枪把手
        collision_primitive_c3 = CollisionBox(Point3(0.20, 0, 0.17),
...
if __name__ == '__main__':
    ...
    cm = grpr.gen_meshmodel(toggle_tcp_frame=True,alpha=0.1)
    for i in cm.cm_list:
        i.show_cdprim()
        i.attach_to(base)
    # Run simulation
