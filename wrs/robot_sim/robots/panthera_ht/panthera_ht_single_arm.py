...
class PantheraHTSglArm(sari.SglArmRobotInterface):
    ...
    def setup_cc(self):
        """设置自碰撞检测，风格参照 PiperSglArm / LRMate。

        基座必须 ``toggle_extcd=False``：否则基座 cdprim 与桌面永久重叠，
        ``is_collided(obstacle_list=[desk])`` 恒为 True，所有 grasp 会被滤掉。
        """
...
if __name__ == '__main__':
    ...
    test_conf = np.array([0.0, 1, 0.5, 0.5, 0.0, 0.0])
