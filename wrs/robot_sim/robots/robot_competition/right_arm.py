...
import wrs.robot_sim.manipulators.realman_arm.RM65_6FB as rman
from wrs.robot_sim.end_effectors.grippers.Omnipicker_gripper.Omnipicker_gripper import Omnipicker
import os
class RealmanR(sari.SglArmRobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="realman_arm", enable_cc=True):
        ...
        ### 将夹爪绑定到机械臂的法兰盘上
        self.end_effector = Omnipicker(pos=self.manipulator.gl_flange_pos,
                                        rotmat=self.manipulator.gl_flange_rotmat, name="injector")
    ...
    def fk(self, jnt_values, toggle_jacobian=False, update=False):
        results =  self._manipulator.fk(jnt_values=jnt_values, toggle_jacobian=toggle_jacobian, update=update)
        if update:
...
if __name__ == '__main__':
    ...
    robot = RealmanR(enable_cc=True)
    robot.gen_meshmodel().attach_to(base)

    tgt_pos = np.array([-0.298,-0.022,-0.143])
