...
class CollisionChecker(object):
    ...
    def enable_extcd_by_id_list(self, id_list, type="from"):
        for uuid in id_list:
            self.cce_dict[uuid].enable_extcd(type=type)

    def disable_extcd_by_id_list(self, id_list, type="from"):
        for uuid in id_list:
            self.cce_dict[uuid].disable_extcd(type=type)

    def enable_innercd_by_id_list(self, id_list, type="into"):
        for uuid in id_list:
            mph.change_cdmask(self.cce_dict[uuid].tfd_cdprim,
                              mph.BITMASK_INNER, action="add", type=type)

    def disable_innercd_by_id_list(self, id_list, type="into"):
        for uuid in id_list:
            mph.change_cdmask(self.cce_dict[uuid].tfd_cdprim,
                              mph.BITMASK_INNER, action="remove", type=type)
    ...
    def set_cdpair(self, lnk_from_list, lnk_into_list):
        ...
        """
        if not lnk_from_list or not lnk_into_list:
            return
    ...
    def set_cdpair_by_ids(self, uuid_from_list, uuid_into_list):
        ...
        date: 20240303
        """
        if not uuid_from_list or not uuid_into_list:
            return
        if len(self.bitmask_pool) == 0:
            raise ValueError("Too many collision pairs! Maximum: 29")
        allocated_bitmask = self.bitmask_pool.pop()
    ...
    def is_collided(self, obstacle_list=None, other_robot_list=None, toggle_contacts=False):
        ...
        if toggle_contacts:
            contact_points = [da.pdvec3_to_npvec3(
                cd_entry.getSurfacePoint(self.cd_pdndp)) for cd_entry in
