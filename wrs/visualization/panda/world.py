...
class World(ShowBase, object):
    ...
    def __init__(self,
        ...
        super().__init__()
        # 真窗口才有 WindowProperties。--no-anime / --export-traj 会开成
        # offscreen GraphicsBuffer，没有 getProperties / requestProperties。
        win = getattr(self, "win", None)
        self._onscreen_win = bool(
            win is not None and hasattr(win, "getProperties") and hasattr(win, "requestProperties")
        )
        if self._onscreen_win:
            winprops = WindowProperties(win.getProperties())
            winprops.setTitle("WRS Robot Planning and Control System")
            win.requestProperties(winprops)
        ...
        # set window size（offscreen 缓冲改不了窗口属性，跳过即可）
        if self._onscreen_win:
