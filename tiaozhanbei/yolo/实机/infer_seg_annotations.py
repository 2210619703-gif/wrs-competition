...
用法:
  python infer_seg_annotations.py --image dataset_learn/0001/0001.jpg --pretty --save-masks --save-pcd
  python infer_seg_annotations.py --image test.jpg --weights best.pt --out pred/annotations.json --save-masks
  python infer_seg_annotations.py --image test.jpg --depth depth_m.npy --state-head runs/classify/state-head-yolo26-v3/weights/best.pt
"""
...
def parse_args():
    ...
    p.add_argument("--image", type=Path, required=True, help="输入图片")
    ...
    p.add_argument("--out", type=Path, default=None, help="输出 JSON；默认 <图目录>/yolo_annotations.json")
...
def load_zh_maps() -> tuple[dict[str, str], dict[str, str]]:
    ...
    try:
        from tiaozhanbei.environment import CATEGORY_ZH, LEAF_DIR_ZH
...
def main():
    ...
    out_path = args.out
    if out_path is None:
        out_path = args.image.parent / "yolo_annotations.json"
    ...
    if pcd_dir is None:
        pcd_dir = out_path.parent / "pointclouds"

    results = model.predict(img_bgr, conf=args.conf, iou=args.iou, verbose=False)[0]
    ...
    for i in range(n_det):
        ...
        if masks is not None and i < len(masks.data):
            ...
            if masks.xy is not None and i < len(masks.xy):
                polygon = polygon_from_xy(masks.xy[i], width, height)
            if len(polygon) < 3:
