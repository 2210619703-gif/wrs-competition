...
def _condition_segment(
    ...
    lo, hi = cfg.joint_limits[last]

    def _j6_over(col) -> float:
        return max((max(lo - v, 0.0) + max(v - hi, 0.0)) for v in col)

    raw_j6 = [float(wp[last]) for wp in flat]
    if WRIST_SHORTEST_PATH:
        col, t_old, t_new = _shortest_wrist(flat, cfg)
        # 抓取位和放置位不是「同一姿态转 180°」。把放置段也折算的话，
        # J6 会被拧到限位外面（这次 181°→172°，越界 94°），而原路径其实只超了几度。
        if _j6_over(col) > _j6_over(raw_j6) + 1e-6:
            col = raw_j6
        elif t_old - t_new > math.radians(1.0):
            warnings.append(
